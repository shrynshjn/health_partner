...
export class WaterService {
  constructor(@InjectModel(Water.name) private waterModel: Model<WaterDocument>) {}
  ...
  async create(userId: string, dto: CreateWaterDto) {
    ...
    await doc.save();
    return { message: 'Water logged successfully', waterId: doc._id, createdAt: doc.createdAt };
  }
  ...
  async findByRange(userId: string, start: Date, end: Date, limit = 100, cursor?: string) {
    ...
    const items = await this.waterModel.find(filter).sort({ _id: -1 }).limit(limit);
