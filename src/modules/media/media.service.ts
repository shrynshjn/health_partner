...
export class MediaService {
  async upload(userId: string, file: Express.Multer.File, type: string, extraPath?: string) {
    ...
    if (enabled) {
      try {
        ...
        const base = process.env.S3_PUBLIC_URL_BASE || `https://${process.env.S3_BUCKET}.s3.${process.env.AWS_REGION}.amazonaws.com`;
        return { url: `${base}/${key}`, mimeType: file.mimetype, size: file.size, key };
      } catch (e) {
        throw new InternalServerErrorException('S3 upload failed: ' + (e as Error).message);
      }
    } else {
      ...
      return { url: `/uploads/${path.basename(diskPath)}`, mimeType: file.mimetype, size: file.size, key: path.basename(diskPath) };
    }
  }

  private buildKey(userId: string, type: string, originalName: string, extraPath?: string) {
