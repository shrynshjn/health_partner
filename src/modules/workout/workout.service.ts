...
export class WorkoutService {
  ...
  async create(userId: string, dto: CreateWorkoutDto) {
    const data: any = { ...dto, userId: new Types.ObjectId(userId), startTime: new Date(dto.startTime) };
    if (dto.endTime) data.endTime = new Date(dto.endTime);
    if (!data.endTime && data.duration) {
      data.endTime = new Date(new Date(dto.startTime).getTime() + data.duration);
    }
    const doc = new this.workoutModel(data);
    await doc.save();
    return { message: 'Workout logged successfully', workoutId: doc._id, createdAt: doc.createdAt };
  }
  ...
  async createBulk(userId: string, items: CreateWorkoutDto[]) {
    ...
    return { message: 'Workouts logged', createdIds: res.map(d => d._id), count: res.length };
  }

  async findByRange(userId: string, start: Date, end: Date, limit = 100, cursor?: string) {
    const filter: any = { userId: new Types.ObjectId(userId), startTime: { $gte: start, $lt: end }, deletedAt: { $exists: false } };
    if (cursor) filter._id = { $lt: new Types.ObjectId(cursor) };
    const items = await this.workoutModel.find(filter).sort({ _id: -1 }).limit(limit);
