...
export class SleepService {
  constructor(@InjectModel(Sleep.name) private sleepModel: Model<SleepDocument>) {}
  ...
  async create(userId: string, dto: CreateSleepDto) {
    ...
    await doc.save();
    return { message: 'Sleep logged successfully', sleepId: doc._id, createdAt: doc.createdAt };
  }
  ...
  async findByRange(userId: string, start: Date, end: Date, limit = 100, cursor?: string) {
    ...
    const items = await this.sleepModel.find(filter).sort({ _id: -1 }).limit(limit);
